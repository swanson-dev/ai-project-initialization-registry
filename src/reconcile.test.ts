...
import { ReconcileOptions } from './types.js';
...
const DEFAULT_RECONCILE_OPTIONS: ReconcileOptions = {
  yes: false,
  json: false,
  verbose: false,
  strict: false,
...
function createOverrideFetchOverrides(manifest: Record<string, unknown>, rawBase: string, ref: string): Record<string, unknown> {
  ...
  const overrides: Record<string, unknown> = {
    ...
    [`${normalizedBase}/scaffolds/standard-planning-plus-code/README.template.md`]:
      'name={{project_name}}\ndescription={{description}}\ntech={{preferred_technology}}\n',
  };
...
async function initializeProject(repoPath: string): Promise<void> {
  void repoPath;
...
test('reconcile falls back when source provenance is absent', async () => {
  await withTempRepo(async (repoPath) => {
    ...
    try {
      ...
      assert.equal(result.status, 'reconciled');
      assert.equal(
        result.notes.includes('selected-assets.json does not include source provenance; reconcile is using compatibility fallback resolution'),
        true,
      );
    } finally {
      restoreFetch();
