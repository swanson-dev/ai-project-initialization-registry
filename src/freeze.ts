...
async function readSelectedAssetsForFreeze(projectRoot: string): Promise<SelectedAssetsPayloadWithOptionalHashes> {
...
function validateFreezePayload(payload: SelectedAssetsPayloadWithOptionalHashes): void {
  ...
  if (
    !Array.isArray(payload.outputs.copied_paths) ||
...
async function collectMissingHashEligiblePaths(projectRoot: string, expectedPaths: string[]): Promise<string[]> {
...
function countHashDifferences(
  previousHashes: SelectedAssetsHashEntry[] | undefined,
  nextHashes: SelectedAssetsHashEntry[],
): number {
  ...
  const allPaths = new Set<string>([...previous.keys(), ...next.keys()]);
  let changed = 0;
...
function buildUpdatedSelectedAssetsPayload(
...
async function writeSelectedAssetsAtomically(
  projectRoot: string,
...
export async function runFreeze(projectRoot: string, options: FreezeOptions): Promise<FreezeResult> {
  try {
...
export function renderFreezeText(
  result: FreezeResult,
  options: Pick<FreezeOptions, 'verbose'>,
): string {
  const lines: string[] = [result.status === 'dry_run' ? 'DRY RUN' : result.status.toUpperCase()];
...
export function getFreezeExitCode(result: FreezeResult): 0 | 2 {
