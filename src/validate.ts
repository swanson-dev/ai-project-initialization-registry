import { InstantiationRule, Manifest, ManifestDefaults, ManifestItem, ManifestMaterialization } from './types.js';
...
function isManifestItemArray(value: unknown): value is ManifestItem[] {
...
function validateDefaults(manifest: Manifest, defaults: ManifestDefaults): void {
  ...
  for (const [defaultsKey, manifestSectionKey] of Object.entries(DEFAULT_KEYS)) {
    const ids = defaults[defaultsKey as keyof typeof DEFAULT_KEYS];
    if (ids === undefined) {
      continue;
    }
...
function validateMaterialization(materialization: ManifestMaterialization): void {
  ...
  let assetGroupRootKeys = new Set<string>();
  if (materialization.asset_group_roots !== undefined) {
    if (
      typeof materialization.asset_group_roots !== 'object' ||
  ...
  if (materialization.copy_raw_asset_groups !== undefined) {
    if (
      !Array.isArray(materialization.copy_raw_asset_groups) ||
  ...
  if (materialization.exclude_globs !== undefined) {
    if (
      !Array.isArray(materialization.exclude_globs) ||
...
function validateInstantiationRules(manifest: Manifest): void {
  ...
  for (const [index, rule] of rules.entries()) {
    ...
    const { template_id, target, required } = rule as InstantiationRule;
