...
import { DoctorOptions, FreezeOptions, SelectedAssetsPayload } from './types.js';

const DEFAULT_FREEZE_OPTIONS: FreezeOptions = {
  yes: false,
  json: false,
  verbose: false,
  strict: false,
...
const DEFAULT_DOCTOR_OPTIONS: DoctorOptions = {
  json: false,
  verbose: false,
  roots: false,
  strict: false,
...
async function writeHashedFixture(root: string): Promise<SelectedAssetsPayload> {
  const payload = await writeSelectedAssetsFixture(root);
  const expectedFiles = [
    ...payload.outputs.copied_paths,
    ...payload.outputs.instantiated_docs,
    ...payload.outputs.metadata_files.filter(
      (item) => item !== '.project/selected-assets.json',
    ),
  ];
...
test('freeze --yes applies update and doctor --hash becomes clean', async () => {
  await withTempProject(async (root) => {
    ...
    const updatedPayload = JSON.parse(
      await fs.readFile(path.join(root, '.project', 'selected-assets.json'), 'utf8'),
    ) as SelectedAssetsPayload;
...
test('freeze rejects legacy provenance', async () => {
  await withTempProject(async (root) => {
    const payload = createSelectedAssetsFixture() as unknown as Record<string, unknown>;
...
test('freeze dry-run no-op reports unchanged payload', async () => {
  await withTempProject(async (root) => {
    ...
    assert.equal(
      ...
      '{"status":"dry_run","provenance_path":".project/selected-assets.json","updated_hashes_count":0,"missing":[],"notes":["selected-assets.json would not change"]}',
    );
    assert.equal(
      renderFreezeText(result, { verbose: true }),
      [
        'DRY RUN',
        '',
        'Updated hashes',
