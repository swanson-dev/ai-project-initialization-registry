...
test('instantiateTemplates aborts if a target already exists and writes no other targets', async () => {
  await withTempRepo(async (repoPath) => {
    ...
    const restoreFetch = installFetchMock({
      'https://raw.githubusercontent.com/swanson-dev/ai-project-initialization/main/file-templates/project/project-brief.md':
        'template:project-brief\n',
      'https://raw.githubusercontent.com/swanson-dev/ai-project-initialization/main/file-templates/project/requirements.md':
      ...
      await assert.rejects(
        () =>
          instantiateTemplates(
            'https://raw.githubusercontent.com/swanson-dev/ai-project-initialization/main',
            createManifest(),
            createWriteContext(),
            repoPath,
          ),
        /Conflict: docs\/00-overview\/project-brief\.md already exists/,
      );
  ...
  await withTempRepo(async (repoPath) => {
    ...
    try {
      await assert.rejects(
        ...
        /Failed to fetch https:\/\/raw\.githubusercontent\.com\/swanson-dev\/ai-project-initialization\/main\/file-templates\/project\/requirements\.md \(404\)/,
      );
