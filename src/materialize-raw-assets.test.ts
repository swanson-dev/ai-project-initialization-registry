...
import { Manifest, ResolvedSelections } from './types.js';
...
function createResolvedSelections(): ResolvedSelections {
  return {
    agentPackIds: ['core'],
...
test('planRawMaterialization sorts outputs, applies excludes, and rejects duplicates', async () => {
  ...
  try {
    const plan = await planRawMaterialization(
      resolveRegistrySource('main'),
      createManifest(),
      createResolvedSelections(),
      'nextjs',
      ['ecommerce'],
      'C:/temp/project',
    );
    ...
    await assert.rejects(
      ...
      /duplicate destination: skills\/documentation-hygiene\/skill\.md/,
    );
  } finally {
    restoreFetch();
  }
...
test('list-based materialization preflight aborts before writes when destination exists', async () => {
  await withTempRepo(async (repoPath) => {
    ...
    try {
      ...
      await assert.rejects(
        () => preflightRawMaterialization(plan),
        /Conflict: skills\/documentation-hygiene\/skill\.md already exists/,
      );
...
test('materializeRawAssets rejects directory-backed override sources', async () => {
  ...
  await assert.rejects(
    ...
    /Directory-backed raw materialization is not supported with registry override sources: agent-packs\/core/,
  );
});
...
test('materializeRawAssets supports file-backed override sources', async () => {
  await withTempRepo(async (repoPath) => {
    ...
    try {
      ...
      assert.equal(
        await fs.readFile(path.join(repoPath, 'file-templates', 'project', 'project-brief.md'), 'utf8'),
        'brief body\n',
      );
    } finally {
      globalThis.fetch = originalFetch;
