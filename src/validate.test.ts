...
test('validateManifest rejects non-object materialization', () => {
  const manifest = createManifest();
  manifest.contract_version = '1';
  manifest.materialization = [] as unknown as Manifest['materialization'];
...
test('validateManifest rejects copy_raw_asset_groups missing from asset_group_roots', () => {
  ...
  assert.throws(
    ...
    /materialization\.copy_raw_asset_groups references missing asset_group_roots key: agent_packs/,
  );
});
...
test('validateManifest rejects invalid copy_raw_asset_groups type', () => {
  ...
  manifest.materialization = {
    ...
    copy_raw_asset_groups: [1] as unknown as string[],
  };
...
test('validateManifest rejects asset_group_roots paths containing dot dot segments', () => {
  ...
  assert.throws(
    () => validateManifest(manifest),
    /materialization\.asset_group_roots\.skills must not contain '\.\.' segments/,
  );
});
...
test('validateManifest rejects invalid project_metadata_dir with dot dot segment', () => {
  ...
  assert.throws(
    () => validateManifest(manifest),
    /materialization\.project_metadata_dir must not contain '\.\.' segments/,
  );
});
...
test('validateManifest rejects invalid exclude_globs type', () => {
  ...
  manifest.contract_version = '1';
  manifest.materialization = {
    exclude_globs: ['**/.gitkeep', 1] as unknown as string[],
  };
...
test('validateManifest rejects non-array instantiation_rules', () => {
  const manifest = createManifest();
  manifest.contract_version = '1';
  manifest.instantiation_rules = {} as unknown as Manifest['instantiation_rules'];
...
test('validateManifest rejects null instantiation_rules', () => {
  const manifest = createManifest();
  manifest.contract_version = '1';
  manifest.instantiation_rules = null as unknown as Manifest['instantiation_rules'];
...
test('validateManifest rejects non-object instantiation rule entries', () => {
  const manifest = createManifest();
  manifest.contract_version = '1';
  manifest.instantiation_rules = ['project-brief'] as unknown as Manifest['instantiation_rules'];
...
test('validateManifest rejects missing template_id in instantiation rules', () => {
  const manifest = createManifest();
  manifest.contract_version = '1';
  manifest.instantiation_rules = [{ target: 'docs/00-overview/project-brief.md' }] as Manifest['instantiation_rules'];
...
test('validateManifest rejects missing target in instantiation rules', () => {
  const manifest = createManifest();
  manifest.contract_version = '1';
  manifest.instantiation_rules = [{ template_id: 'project-brief' }] as Manifest['instantiation_rules'];
...
test('validateManifest rejects non-boolean required in instantiation rules', () => {
  ...
  ] as unknown as Manifest['instantiation_rules'];
...
test('validateManifest rejects unknown template_id in instantiation rules', () => {
  ...
  assert.throws(
    () => validateManifest(manifest),
    /instantiation_rules\[0\]\.template_id references unknown file template id: missing/,
  );
});
...
test('validateManifest rejects absolute instantiation rule targets', () => {
  ...
  assert.throws(
    () => validateManifest(manifest),
    /instantiation_rules\[0\]\.target must be a relative path: \/docs\/00-overview\/project-brief\.md/,
  );
});
...
test('validateManifest rejects dot dot segments in instantiation rule targets', () => {
  ...
  assert.throws(
    () => validateManifest(manifest),
    /instantiation_rules\[0\]\.target must not contain '\.\.' segments: docs\/\.\.\/project-brief\.md/,
  );
});
...
test('validateManifest rejects instantiation rule targets outside docs', () => {
  ...
  assert.throws(
    () => validateManifest(manifest),
    /instantiation_rules\[0\]\.target must be under docs\/: app\/project-brief\.md/,
  );
});
...
test('validateManifest rejects duplicate normalized instantiation rule targets', () => {
  ...
  assert.throws(
    () => validateManifest(manifest),
    /instantiation_rules contains duplicate target: docs\/00-overview\/project-brief\.md/,
  );
});
