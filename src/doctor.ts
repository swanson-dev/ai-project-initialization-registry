...
type SelectedAssetsPayloadWithHashes = SelectedAssetsPayload & {
...
function createResult(
  status: DoctorStatus,
  provenance: DoctorResult['provenance'],
  missing: string[],
  extra: string[],
  notes: string[],
): DoctorResult {
...
export function normalizeProjectRelativePath(relPath: string): string {
...
async function readSelectedAssets(projectRoot: string): Promise<SelectedAssetsPayloadWithHashes> {
  ...
  return payload as SelectedAssetsPayloadWithHashes;
}

export function buildExpectedPathSet(payload: SelectedAssetsPayload): Set<string> {
...
export async function collectMissingExpectedPaths(projectRoot: string, expectedPaths: string[]): Promise<string[]> {
...
export function getKnownRoots(): string[] {
  return ['.project', 'agent-packs', 'agents', 'docs', 'file-templates', 'skills', 'tech-stacks'];
}

async function listFilesUnderRoot(projectRoot: string, rootRel: string): Promise<string[]> {
...
export async function collectExtraPaths(
  projectRoot: string,
...
function hasHashData(payload: SelectedAssetsPayloadWithHashes): boolean {
  return Array.isArray(payload.outputs.hashes) && payload.outputs.hashes.length > 0;
}

async function verifyHashesIfRequested(
  payload: SelectedAssetsPayloadWithHashes,
...
export async function runDoctor(projectRoot: string, options: DoctorOptions): Promise<DoctorResult> {
  let payload: SelectedAssetsPayloadWithHashes | null = null;
  let provenance: DoctorResult['provenance'] = {
  ...
  try {
    ...
    const status: DoctorStatus = missing.length > 0 || extra.length > 0 || hashNotes.length > 0 ? 'drift' : 'clean';
...
export function renderDoctorText(
  result: DoctorResult,
  options: Pick<DoctorOptions, 'verbose' | 'strict'>,
): string {
  if (result.status === 'clean' && !options.verbose) {
    return 'CLEAN';
  }

  const lines: string[] = [result.status.toUpperCase()];
...
export function getDoctorExitCode(result: DoctorResult): 0 | 1 | 2 {
