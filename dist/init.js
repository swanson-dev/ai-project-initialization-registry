...
import { instantiatePlannedTemplates, planTemplateInstantiation, preflightTemplateInstantiation, } from './instantiate-templates.js';
import { materializeRawAssets, planRawMaterialization, preflightRawMaterialization, } from './materialize-raw-assets.js';
import { writeLegacyBootstrapLock, writeMetadataFiles } from './metadata.js';
import { promptSelections } from './prompt.js';
import { buildBootstrapLockPayload, buildSelectedAssetsPayload, writeBootstrapLock, writeSelectedAssets, } from './provenance.js';
import { resolveSelections } from './resolve-selections.js';
import { fillTemplate } from './template.js';
import { validateManifest } from './validate.js';
import { createWriteContext, recordCreatedFile, rollbackWriteContext } from './write-context.js';
...
async function ensureInitFileDoesNotExist(relativePath) {
    try {
...
async function preflightStaticWriteTargets(contractVersion) {
    const targets = [
...
async function writeReadme(rawBase, description, preferredTechnology, context) {
...
export async function runInit(options) {
    ...
    try {
        ...
        const rawMaterializationPlan = await planRawMaterialization(source, manifest, resolvedSelections, selections.preferredTechnology, selections.productPackId ? [selections.productPackId] : []);
        ...
        if (manifest.contract_version === '1') {
            ...
            createdFiles.push(await writeLegacyBootstrapLock({
                description: selections.description,
                preferredTechnology: selections.preferredTechnology,
                productPackId: selections.productPackId,
                selectedSkillIds: selections.selectedSkillIds,
