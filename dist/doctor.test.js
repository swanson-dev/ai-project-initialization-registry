...
async function writeHashedSelectedAssetsFixture(root) {
...
test('doctor reports clean project and renders clean text output', async () => {
    await withTempProject(async (root) => {
        ...
        assert.equal(renderDoctorText(result, { verbose: true, strict: false }), [
            'CLEAN',
            '',
            'Missing files',
            '  none',
...
test('doctor reports missing provenance as error', async () => {
    await withTempProject(async (root) => {
        ...
        assert.equal(renderDoctorText(result, { verbose: false, strict: false }), ['ERROR', '', 'Provenance problems', '  .project/selected-assets.json is missing'].join('\n'));
    });
});
test('doctor json output keeps stable key order and sorted arrays', async () => {
    await withTempProject(async (root) => {
        ...
        assert.equal(JSON.stringify(result), '{"status":"clean","provenance":{"path":".project/selected-assets.json","registry_version":"0.2.0","published_at":"2026-03-01T00:00:00Z","contract_version":"1"},"missing":[],"extra":[],"notes":[]}');
    });
});
