...
export function ensureRelativePathSafe(relPath) {
...
export function matchesExcludeGlobs(relPath, excludeGlobs) {
...
function findById(items, id, group) {
...
function getSelectedMaterializationEntries(manifest, resolvedSelections, techStackRecipeId, productPackIds) {
        ...
        switch (group) {
            ...
            case 'product_type_packs':
                for (const id of productPackIds) {
                    const productTypePacks = manifest.product_type_packs ?? [];
...
async function enumerateFilesUnderPath(source, manifestEntryPath, excludeGlobs) {
...
export async function planRawMaterialization(source, manifest, resolvedSelections, techStackRecipeId, productPackIds, projectRoot = process.cwd()) {
...
export async function preflightRawMaterialization(plan) {
...
export async function materializeRawAssets(source, plan, context, projectRoot = process.cwd()) {
    const createdFiles = [];
    for (const item of plan) {
