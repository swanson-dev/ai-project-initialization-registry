...
test('runInit aborts when app already exists', async () => {
    await withTempRepo(async (repoPath) => {
        ...
        await assert.rejects(() => runInit({ ref: 'main', yes: true, debug: false }), /Validation failed: app must not exist/);
    });
});
test('runInit aborts when docs already exists', async () => {
    await withTempRepo(async (repoPath) => {
        await fs.mkdir(path.join(repoPath, 'docs'));
        await assert.rejects(() => runInit({ ref: 'main', yes: true, debug: false }), /Validation failed: docs must not exist/);
    });
});
test('runInit aborts on defaults validation failure before writing files', async () => {
    await withTempRepo(async (repoPath) => {
        ...
        try {
            await assert.rejects(() => runInit({ ref: 'main', yes: true, debug: false }), /defaults\.skills references unknown id: missing-skill/);
...
test('runInit logs resolved selections as single-line json in debug mode', async () => {
    await withTempRepo(async (repoPath) => {
        ...
        try {
            ...
            assert.equal(resolvedLine, JSON.stringify({
                resolvedSelections: {
                    agentPackIds: ['core'],
                ...
                instantiation_rules: manifest.instantiation_rules,
            }));
            for (const template of TEMPLATE_FIXTURES) {
            ...
            assert.equal(await fs.readFile(path.join(repoPath, 'agent-packs', 'core', 'agent-guidelines.md'), 'utf8'), 'core guidelines\n');
            assert.equal(await fs.readFile(path.join(repoPath, 'skills', 'documentation-hygiene', 'skill.md'), 'utf8'), 'skill body\n');
            assert.equal(await fs.readFile(path.join(repoPath, 'tech-stacks', 'nextjs', 'recipe.md'), 'utf8'), 'next recipe\n');
            ...
            assert.deepEqual(selectedAssets.materialization, manifest.materialization);
            assert.deepEqual(selectedAssets.outputs.copied_paths, [
            ...
            assert.deepEqual(selectedAssets.outputs.instantiated_docs, TEMPLATE_FIXTURES.map((template) => template.target));
            assert.deepEqual(selectedAssets.outputs.metadata_files, [
                '.project/bootstrap.lock',
                '.project/project.config.json',
                '.project/selected-assets.json',
            ]);
            const hashes = selectedAssets.outputs.hashes;
            assert.ok(Array.isArray(hashes));
            assert.deepEqual(hashes.map((entry) => entry.path), [
                '.project/project.config.json',
                'agent-packs/core/agent-guidelines.md',
                'agent-packs/core/OUTPUT_RULES.md',
            ...
            const agentGuidelinesHash = createHash('sha256')
                ...
                .digest('hex');
            assert.equal(hashes.find((entry) => entry.path === 'agent-packs/core/agent-guidelines.md')?.sha256, agentGuidelinesHash);
            const bootstrapLock = JSON.parse(await fs.readFile(bootstrapLockPath, 'utf8'));
...
test('runInit aborts before writes when selected-assets target already exists', async () => {
    await withTempRepo(async (repoPath) => {
        ...
        try {
            await assert.rejects(() => runInit({ ref: 'main', yes: true, debug: false }), /Conflict: \.project\/selected-assets\.json already exists/);
            const files = await fs.readdir(repoPath);
...
test('runInit rolls back selected-assets when bootstrap lock write fails', async (t) => {
    await withTempRepo(async (repoPath) => {
        ...
        try {
            await assert.rejects(() => runInit({ ref: 'main', yes: true, debug: false }), /Simulated bootstrap lock write failure/);
            const files = await fs.readdir(repoPath);
...
test('runInit rolls back raw materialization when a copy fails mid-plan', async (t) => {
    await withTempRepo(async (repoPath) => {
        ...
        try {
            await assert.rejects(() => runInit({ ref: 'main', yes: true, debug: false }), /Simulated raw materialization write failure/);
...
test('runInit rolls back when hash computation fails', async (t) => {
    await withTempRepo(async (repoPath) => {
        ...
        let failureTriggered = false;
        t.mock.method(fs, 'readFile', async (target, options) => {
            if (typeof target === 'string' &&
        ...
        try {
            await assert.rejects(() => runInit({ ref: 'main', yes: true, debug: false }), /Simulated hash computation read failure/);
            assert.equal(failureTriggered, true);
            const files = await fs.readdir(repoPath);
            assert.deepEqual(files.sort(), ['.git', 'sentinel.txt']);
            assert.equal(await fs.readFile(sentinelPath, 'utf8'), 'keep-me\n');
...
test('runInit logs null instantiation_rules in legacy debug mode', async () => {
    await withTempRepo(async (repoPath) => {
        ...
        try {
            await runInit({ ref: 'main', yes: true, debug: true });
            const resolvedLine = logs.find((line) => line.startsWith('{"resolvedSelections":'));
            assert.equal(resolvedLine, '{"resolvedSelections":{"agentPackIds":["core"],"skillIds":["documentation-hygiene"],"registryDocIds":[],"fileTemplateIds":[]},"materialization":null,"instantiation_rules":null}');
            await assert.rejects(() => fs.access(path.join(repoPath, '.project', 'selected-assets.json')));
            const legacyBootstrap = JSON.parse(await fs.readFile(path.join(repoPath, '.project', 'bootstrap.lock'), 'utf8'));
